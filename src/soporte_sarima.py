...
class SARIMAModel:
    ...
    def dividir_train_test(self, df, porc_test):
        datos_en_test = int(df.shape[0]*porc_test)
    ...
    def evaluar_modelos(self, y_train, y_test, parametros, diferenciacion, df_length, variable):
        ...
        print(f"Los mejores parámetros son: {self.best_params} con un RMSE de {round(self.best_model['RMSE'],2)}")
    ...
    def predict_sarima(self, df_final, order, seasonal_order, steps, col_predecir):
        modelo_sarima = SARIMAX(df_final,
                            order=order, 
                            seasonal_order=seasonal_order).fit()

        df_predict = pd.DataFrame(modelo_sarima.forecast(steps=steps))
        df_predict["predicted_mean"] = df_predict["predicted_mean"].astype("int")

        plt.figure(figsize=(15,5))
        sns.lineplot(x=df_final.index, y=col_predecir, data=df_final, label=col_predecir)
        sns.lineplot(x=df_predict.index, y="predicted_mean", data=df_predict, linestyle="--", color="red", label="Predichos")
